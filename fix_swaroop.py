...
photo_path = r'C:\Users\pc\.gemini\antigravity\brain\ab98e414-96f1-47f0-bd3d-25b94e55a072\.user_uploaded\media_1789723135001.jpg'
src = Image.open(photo_path).convert('RGB')
w, h = src.size

# Bottom half, left devotee with hair, glasses, singing into mic
...
profile_sq.save(os.path.join(images_dir, 'swaroop_krishna_prabhu.jpg'), quality=95)
...
card_file = 'radharani_dupatta_swaroop_krishna.png'
card.save(os.path.join(images_dir, card_file), 'PNG')

subprocess.run(['git', 'add', '.'], cwd=base_dir, check=True)
subprocess.run(['git', 'commit', '-m', 'Update HG Swaroop Krishna Prabhu (bottom left devotee with hair and glasses)'], cwd=base_dir, check=True)
subprocess.run(['git', 'push', 'origin', 'main'], cwd=base_dir, check=True)
print('SUCCESS_UPDATED_SWAROOP_KRISHNA')
